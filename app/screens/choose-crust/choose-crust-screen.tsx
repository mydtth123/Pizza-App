...
export const ChooseCrustScreen: FC<StackScreenProps<NavigatorParamList, "choose-crust">> = observer(
  ({ route, navigation }) => {
    const { size = "", price } = route.params || {}
    ...
    const buttonData = useMemo(() => {
      return [
        ...
        },
        {
          ...
          title: "Thick",
          active: crust === "thick",
          onPress: () => setCrust("thick"),
        },
      ]
    }, [crust])

    const pizzaPrice = useMemo<number>(() => {
      if (crust === "thin") return price + 2
      if (crust === "thick") return price + 4
      return price
    }, [price, crust])

    const onNextPress = () => {
      navigation.navigate("choose-topping", { size, crust, price: pizzaPrice })
    }

    return (
      <View testID="ChooseCrustScreen" style={FULL}>
        <Screen
          style={CONTAINER}
          preset="scroll"
        ...
        >
          ...
          <View style={SUB_CONTAINER}>
            ...
            <Text style={OPTIONS_TEXT}>
              <Text style={OPTIONS_TEXT_WHITE}>{size}</Text>, crust, toppings
            </Text>
          </View>
          <PizzaItem size={size} crust={crust} showCrustText />
          <View style={CHOOSE_FORM}>
            <Text style={CHOOSE_TEXT}>
              Choose your <Text style={CHOOSE_TEXT_BOLD}>crust</Text>
            </Text>
